--!strict

-- Server-authoritative fishing and lakeside shop definitions.

local Fishing = {}

Fishing.fish = {
	wakasagi = {
		id = 'wakasagi',
		name = 'Wakasagi Smelt',
		japaneseName = 'ワカサギ',
		rarity = 'Common',
		weight = 32,
		minLength = 7,
		maxLength = 15,
		baseValue = 140,
		color = Color3.fromRGB(176, 193, 199),
	},
	moroko = {
		id = 'moroko',
		name = 'Tamoroko',
		japaneseName = 'タモロコ',
		rarity = 'Common',
		weight = 26,
		minLength = 6,
		maxLength = 12,
		baseValue = 110,
		color = Color3.fromRGB(185, 180, 160),
	},
	kamatsuka = {
		id = 'kamatsuka',
		name = 'Japanese Gudgeon',
		japaneseName = 'カマツカ',
		rarity = 'Common',
		weight = 22,
		minLength = 11,
		maxLength = 20,
		baseValue = 190,
		color = Color3.fromRGB(160, 148, 125),
	},
	crucian_carp = {
		id = 'crucian_carp',
		name = 'Crucian Carp',
		japaneseName = 'フナ',
		rarity = 'Common',
		weight = 22,
		minLength = 18,
		maxLength = 34,
		baseValue = 280,
		color = Color3.fromRGB(145, 137, 99),
	},
	bluegill = {
		id = 'bluegill',
		name = 'Bluegill',
		japaneseName = 'ブルーギル',
		rarity = 'Uncommon',
		weight = 18,
		minLength = 14,
		maxLength = 26,
		baseValue = 360,
		color = Color3.fromRGB(100, 135, 140),
	},
	black_bass = {
		id = 'black_bass',
		name = 'Black Bass',
		japaneseName = 'ブラックバス',
		rarity = 'Uncommon',
		weight = 14,
		minLength = 25,
		maxLength = 55,
		baseValue = 560,
		color = Color3.fromRGB(91, 115, 78),
	},
	common_carp = {
		id = 'common_carp',
		name = 'Common Carp',
		japaneseName = 'コイ',
		rarity = 'Uncommon',
		weight = 12,
		minLength = 35,
		maxLength = 80,
		baseValue = 680,
		color = Color3.fromRGB(173, 128, 72),
	},
	ayu = {
		id = 'ayu',
		name = 'Sweetfish',
		japaneseName = 'アユ',
		rarity = 'Uncommon',
		weight = 10,
		minLength = 15,
		maxLength = 30,
		baseValue = 750,
		color = Color3.fromRGB(165, 180, 120),
	},
	rainbow_trout = {
		id = 'rainbow_trout',
		name = 'Rainbow Trout',
		japaneseName = 'ニジマス',
		rarity = 'Rare',
		weight = 8,
		minLength = 28,
		maxLength = 60,
		baseValue = 1050,
		color = Color3.fromRGB(127, 163, 159),
	},
	catfish = {
		id = 'catfish',
		name = 'Japanese Catfish',
		japaneseName = 'ナマズ',
		rarity = 'Rare',
		weight = 6,
		minLength = 40,
		maxLength = 75,
		baseValue = 1300,
		color = Color3.fromRGB(80, 75, 70),
	},
	eel = {
		id = 'eel',
		name = 'Japanese Eel',
		japaneseName = 'ウナギ',
		rarity = 'Rare',
		weight = 5,
		minLength = 40,
		maxLength = 90,
		baseValue = 1600,
		color = Color3.fromRGB(74, 75, 55),
	},
	cherry_salmon = {
		id = 'cherry_salmon',
		name = 'Cherry Salmon',
		japaneseName = 'ヤマメ',
		rarity = 'Epic',
		weight = 3,
		minLength = 25,
		maxLength = 45,
		baseValue = 2200,
		color = Color3.fromRGB(195, 140, 150),
	},
	lake_trout = {
		id = 'lake_trout',
		name = 'Lake Iwana',
		japaneseName = 'イワナ',
		rarity = 'Epic',
		weight = 2,
		minLength = 30,
		maxLength = 55,
		baseValue = 2800,
		color = Color3.fromRGB(130, 145, 120),
	},
	golden_koi = {
		id = 'golden_koi',
		name = 'Legendary Golden Koi',
		japaneseName = '金色の主コイ',
		rarity = 'Legendary',
		weight = 1,
		minLength = 70,
		maxLength = 110,
		baseValue = 5500,
		color = Color3.fromRGB(245, 195, 45),
	},
}

Fishing.junk = {
	old_boot = {
		id = 'old_boot',
		name = 'Old Boot',
		rarity = 'Junk',
		weight = 5,
		baseValue = 5,
	},
	empty_can = {
		id = 'empty_can',
		name = 'Empty Can',
		rarity = 'Junk',
		weight = 4,
		baseValue = 3,
	},
}

Fishing.shops = {
	fishing_supply = {
		id = 'fishing_supply',
		name = 'Suwako Fishing Supply (釣具店)',
		items = {
			{ id = 'fishing_rod', name = 'Beginner Fishing Rod', price = 0, amount = 1 },
			{ id = 'pro_fishing_rod', name = 'Pro Carbon Fishing Rod', price = 1800, amount = 1 },
			{ id = 'worm_bait', name = 'Worm Bait ×5 (無料)', price = 0, amount = 5 },
			{ id = 'shrimp_bait', name = 'River Shrimp Bait ×5', price = 150, amount = 5 },
			{ id = 'golden_lure', name = 'Golden Spinner Lure', price = 800, amount = 1 },
			{ id = 'tackle_box', name = 'Lake Tackle Box', price = 500, amount = 1 },
			{ id = 'fisherman_tea', name = 'Hot Fisherman Green Tea', price = 130, amount = 1 },
			{ id = 'umeboshi_onigiri', name = 'Umeboshi Onigiri', price = 150, amount = 1 },
		},
	},
	ice_cream = {
		id = 'ice_cream',
		name = 'Suwako Ice Cream (アイスクリーム)',
		items = {
			{ id = 'vanilla_ice_cream', name = 'Vanilla Ice Cream', price = 250, amount = 1 },
			{ id = 'matcha_ice_cream', name = 'Matcha Ice Cream', price = 300, amount = 1 },
			{ id = 'strawberry_ice_cream', name = 'Strawberry Gelato', price = 300, amount = 1 },
			{ id = 'chocolate_ice_cream', name = 'Chocolate Soft Serve', price = 280, amount = 1 },
			{ id = 'apple_sorbet', name = 'Shinshu Apple Sorbet', price = 320, amount = 1 },
			{ id = 'ice_coffee', name = 'Iced Japanese Coffee', price = 200, amount = 1 },
		},
	},
	island_festival = {
		id = 'island_festival',
		name = 'Hatsushima Festival Stall (島の売店)',
		items = {
			{ id = 'dango', name = 'Hanami Dango', price = 200, amount = 1 },
			{ id = 'yakisoba', name = 'Festival Yakisoba', price = 450, amount = 1 },
			{ id = 'taiyaki', name = 'Warm Taiyaki', price = 220, amount = 1 },
			{ id = 'onigiri', name = 'Salmon Onigiri', price = 160, amount = 1 },
			{ id = 'ramune', name = 'Cold Ramune', price = 180, amount = 1 },
			{ id = 'matcha_tea', name = 'Iced Matcha Tea', price = 220, amount = 1 },
			{ id = 'sparkler_pack', name = 'Sparkler Pack', price = 300, amount = 1 },
		},
	},
	seven_eleven_drinks = {
		id = 'seven_eleven_drinks',
		name = '7-Eleven Drink Cooler (ドリンク冷蔵庫)',
		isBasket = true,
		items = {
			{ id = 'seven_cafe_latte', name = '7-Café Iced Latte', price = 210, amount = 1 },
			{ id = 'green_tea_bottle', name = 'Shinshu Green Tea 500ml', price = 140, amount = 1 },
			{ id = 'pocari_sweat', name = 'Pocari Sweat 500ml', price = 160, amount = 1 },
			{ id = 'ramune', name = 'Cold Ramune Soda', price = 180, amount = 1 },
			{ id = 'ice_coffee', name = 'Iced Japanese Coffee', price = 200, amount = 1 },
			{ id = 'fisherman_tea', name = 'Hot Fisherman Green Tea', price = 130, amount = 1 },
		},
	},
	seven_eleven_food = {
		id = 'seven_eleven_food',
		name = '7-Eleven Bento & Snacks (お弁当・スナック棚)',
		isBasket = true,
		items = {
			{ id = 'nanachiki', name = 'Nanachiki Crispy Chicken', price = 220, amount = 1 },
			{ id = 'tamago_sandwich', name = 'Egg Salad Sandwich', price = 260, amount = 1 },
			{ id = 'tuna_mayo_onigiri', name = 'Tuna Mayo Onigiri', price = 150, amount = 1 },
			{ id = 'katsu_curry', name = 'Katsu Curry Bento', price = 580, amount = 1 },
			{ id = 'melonpan', name = 'Fluffy Melonpan', price = 140, amount = 1 },
			{ id = 'pocky_box', name = 'Chocolate Pocky', price = 180, amount = 1 },
			{ id = 'umeboshi_onigiri', name = 'Umeboshi Onigiri', price = 150, amount = 1 },
		},
	},
	seven_eleven_sundries = {
		id = 'seven_eleven_sundries',
		name = '7-Eleven Sundries & Bait (日用品・釣具)',
		isBasket = true,
		items = {
			{ id = 'worm_bait', name = 'Worm Bait ×5', price = 100, amount = 5 },
			{ id = 'shrimp_bait', name = 'River Shrimp Bait ×5', price = 150, amount = 5 },
			{ id = 'sparkler_pack', name = 'Hanabi Sparkler Pack', price = 300, amount = 1 },
		},
	},
	seven_eleven_slurpee = {
		id = 'seven_eleven_slurpee',
		name = '7-Eleven Slurpee & Drink Bar (スラーピー・ドリンクバー)',
		isBasket = true,
		items = {
			{ id = 'slurpee_cherry', name = 'Wild Cherry Slurpee (チェリースラーピー)', price = 160, amount = 1 },
			{ id = 'slurpee_blue_ice', name = 'Blue Raspberry Slurpee (ブルーアイススラーピー)', price = 160, amount = 1 },
			{ id = 'fountain_coca_cola', name = 'Fountain Coca-Cola (ファウンテンコーラ)', price = 150, amount = 1 },
			{ id = 'fountain_melon_soda', name = 'Fountain Melon Soda (メロンソーダ)', price = 150, amount = 1 },
			{ id = 'seven_cafe_latte', name = '7-Café Iced Latte', price = 210, amount = 1 },
		},
	},
	seven_eleven = {
		id = 'seven_eleven',
		name = '7-Eleven Cashier (レジカウンター)',
		isBasket = false,
		items = {
			{ id = 'nanachiki', name = 'Nanachiki Crispy Chicken', price = 220, amount = 1 },
			{ id = 'tamago_sandwich', name = 'Egg Salad Sandwich', price = 260, amount = 1 },
			{ id = 'tuna_mayo_onigiri', name = 'Tuna Mayo Onigiri', price = 150, amount = 1 },
			{ id = 'katsu_curry', name = 'Katsu Curry Bento', price = 580, amount = 1 },
			{ id = 'melonpan', name = 'Fluffy Melonpan', price = 140, amount = 1 },
			{ id = 'pocky_box', name = 'Chocolate Pocky', price = 180, amount = 1 },
			{ id = 'slurpee_cherry', name = 'Wild Cherry Slurpee', price = 160, amount = 1 },
			{ id = 'fountain_coca_cola', name = 'Fountain Coca-Cola', price = 150, amount = 1 },
			{ id = 'seven_cafe_latte', name = '7-Café Iced Latte', price = 210, amount = 1 },
			{ id = 'green_tea_bottle', name = 'Shinshu Green Tea 500ml', price = 140, amount = 1 },
			{ id = 'worm_bait', name = 'Worm Bait ×5', price = 100, amount = 5 },
			{ id = 'shrimp_bait', name = 'River Shrimp Bait ×5', price = 150, amount = 5 },
			{ id = 'sparkler_pack', name = 'Hanabi Sparkler Pack', price = 300, amount = 1 },
		},
	},
	suwa_sushi = {
		id = 'suwa_sushi',
		name = 'Suwa Sushi Restaurant (諏訪すし店)',
		isBasket = false,
		items = {
			{ id = 'sushi_salmon_nigiri', name = 'Salmon Nigiri (サーモン握り)', price = 420, amount = 1 },
			{ id = 'sushi_maguro_nigiri', name = 'Tuna Nigiri (マグロ握り)', price = 480, amount = 1 },
			{ id = 'sushi_california_roll', name = 'California Roll (カリフォルニアロール)', price = 380, amount = 1 },
			{ id = 'sushi_sashimi_combo', name = 'Sashimi Platter (刺身盛り合わせ)', price = 780, amount = 1 },
			{ id = 'sushi_tempura_platter', name = 'Crispy Tempura (天ぷら盛り合わせ)', price = 550, amount = 1 },
			{ id = 'sushi_miso_soup', name = 'Tofu Miso Soup (豆腐の味噌汁)', price = 150, amount = 1 },
			{ id = 'sushi_edamame', name = 'Steamed Edamame (枝豆)', price = 200, amount = 1 },
			{ id = 'sushi_green_tea', name = 'Hot Sencha Green Tea (熱い煎茶)', price = 120, amount = 1 },
		},
	},
}

Fishing.itemNames = {
	sushi_salmon_nigiri = 'Salmon Nigiri',
	sushi_maguro_nigiri = 'Tuna Nigiri',
	sushi_california_roll = 'California Roll',
	sushi_sashimi_combo = 'Sashimi Platter',
	sushi_tempura_platter = 'Crispy Tempura',
	sushi_miso_soup = 'Tofu Miso Soup',
	sushi_edamame = 'Steamed Edamame',
	sushi_green_tea = 'Hot Sencha Green Tea',
	coca_cola = 'Coca-Cola',
	pepsi = 'Pepsi',
	sprite = 'Sprite',
	mountain_dew = 'Mountain Dew',
	dr_pepper = 'Dr Pepper',
	fresh_milk = 'Fresh Milk',
	orange_juice = 'Orange Juice',
	doritos_chips = 'Doritos Chips',
	hersheys_chocolate = "Hershey's Chocolate",
	choco_chip_cookie = 'Choco Chip Cookie',
	golden_waffle = 'Golden Waffle',
	bloxy_snack = 'Bloxy Snack',
	slurpee_cherry = 'Wild Cherry Slurpee',
	slurpee_blue_ice = 'Blue Raspberry Slurpee',
	fountain_coca_cola = 'Fountain Coca-Cola',
	fountain_melon_soda = 'Fountain Melon Soda',
	nanachiki = 'Nanachiki Crispy Chicken',
	tamago_sandwich = 'Egg Salad Sandwich',
	tuna_mayo_onigiri = 'Tuna Mayo Onigiri',
	katsu_curry = 'Katsu Curry Bento',
	melonpan = 'Fluffy Melonpan',
	pocky_box = 'Chocolate Pocky',
	seven_cafe_latte = '7-Café Iced Latte',
	green_tea_bottle = 'Shinshu Green Tea 500ml',
	pocari_sweat = 'Pocari Sweat 500ml',
	fishing_rod = 'Beginner Fishing Rod',
	pro_fishing_rod = 'Pro Carbon Fishing Rod',
	worm_bait = 'Worm Bait',
	shrimp_bait = 'River Shrimp Bait',
	golden_lure = 'Golden Spinner Lure',
	tackle_box = 'Lake Tackle Box',
	fisherman_tea = 'Hot Fisherman Green Tea',
	umeboshi_onigiri = 'Umeboshi Onigiri',
	vanilla_ice_cream = 'Vanilla Ice Cream',
	matcha_ice_cream = 'Matcha Ice Cream',
	strawberry_ice_cream = 'Strawberry Gelato',
	chocolate_ice_cream = 'Chocolate Soft Serve',
	apple_sorbet = 'Shinshu Apple Sorbet',
	ice_coffee = 'Iced Japanese Coffee',
	dango = 'Hanami Dango',
	yakisoba = 'Festival Yakisoba',
	taiyaki = 'Warm Taiyaki',
	onigiri = 'Salmon Onigiri',
	ramune = 'Cold Ramune',
	matcha_tea = 'Iced Matcha Tea',
	sparkler_pack = 'Sparkler Pack',
	old_boot = 'Old Boot',
	empty_can = 'Empty Can',
	wakasagi = 'Wakasagi Smelt',
	moroko = 'Tamoroko',
	kamatsuka = 'Japanese Gudgeon',
	crucian_carp = 'Crucian Carp',
	bluegill = 'Bluegill',
	black_bass = 'Black Bass',
	common_carp = 'Common Carp',
	ayu = 'Sweetfish',
	rainbow_trout = 'Rainbow Trout',
	catfish = 'Japanese Catfish',
	eel = 'Japanese Eel',
	cherry_salmon = 'Cherry Salmon',
	lake_trout = 'Lake Iwana',
	golden_koi = 'Legendary Golden Koi',
}

return Fishing
