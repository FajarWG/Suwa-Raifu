...
local function mountBebeqLaunchers(deckTopY: number)
	local bebeq = workspace:FindFirstChild('BebeqFireworkSystem')
	if not bebeq then
		return
	end
	for _, item in bebeq:GetChildren() do
		if item:IsA('Model') and item.Name == 'FireworkLauncher' then
			local base = item:FindFirstChild('Base')
			if base and base:IsA('BasePart') then
				local targetY = deckTopY + base.Size.Y / 2
				local diff = targetY - base.Position.Y
				if math.abs(diff) > 0.01 then
					item:PivotTo(item:GetPivot() + Vector3.new(0, diff, 0))
				end
				base.Anchored = true
			end
...
local function buildLaunchBattery(): { Vector3 }
	...
	local deckCenterX = (BATTERY_X_MIN + BATTERY_X_MAX) / 2 -- 4
	local deckTopY = BATTERY_Y + 0.8 -- 2.4

	-- 1. Main Heavy Barge (Z = -707, holds the giant steel mortar tubes)
	local barge = Instance.new('Part')
	barge.Name = 'Barge'
	barge.Size = Vector3.new(156, 1.6, 16)
	barge.CFrame = CFrame.new(deckCenterX, BATTERY_Y, -707)
	barge.Color = Color3.fromRGB(48, 42, 38)
	barge.Material = Enum.Material.WoodPlanks
	barge.Anchored = true
	barge.Parent = battery

	-- 2. Wooden Firing Pier / Dock (Z = -695, sits directly under the front row of petasan)
	local firingPier = Instance.new('Part')
	firingPier.Name = 'FiringPier'
	firingPier.Size = Vector3.new(144, 1.6, 8)
	firingPier.CFrame = CFrame.new(deckCenterX, BATTERY_Y, -695)
	firingPier.Color = Color3.fromRGB(56, 48, 40)
	firingPier.Material = Enum.Material.WoodPlanks
	firingPier.Anchored = true
	firingPier.Parent = battery

	-- Mid-connection deck linking FiringPier to Barge
	local midDeck = Instance.new('Part')
	midDeck.Name = 'MidConnectionDeck'
	midDeck.Size = Vector3.new(144, 1.6, 4)
	midDeck.CFrame = CFrame.new(deckCenterX, BATTERY_Y, -701)
	midDeck.Color = Color3.fromRGB(52, 45, 39)
	midDeck.Material = Enum.Material.WoodPlanks
	midDeck.Anchored = true
	midDeck.Parent = battery

	-- 3. Straight Wooden Pier Walkway from shore to the barge ("lurusan kayu pelabuhan")
	-- Extends from island shore edge (Z = -682) out across the water to meet the dock (Z = -699)
	local walkwayLength = 18
	local walkwayCenterZ = -690.5
	local walkway = Instance.new('Part')
	walkway.Name = 'ShorePierWalkway'
	walkway.Size = Vector3.new(8, 1.6, walkwayLength)
	walkway.CFrame = CFrame.new(deckCenterX, BATTERY_Y, walkwayCenterZ)
	walkway.Color = Color3.fromRGB(58, 49, 42)
	walkway.Material = Enum.Material.WoodPlanks
	walkway.Anchored = true
	walkway.Parent = battery

	-- 4. Shore Ramp (walkway ramp from island terrain onto the wooden pier)
	local ramp = Instance.new('Part')
	ramp.Name = 'ShoreRamp'
	ramp.Size = Vector3.new(8, 0.7, 4.5)
	ramp.CFrame = CFrame.new(deckCenterX, BATTERY_Y + 0.35, -680.5)
		* CFrame.Angles(math.rad(-6), 0, 0)
	ramp.Color = Color3.fromRGB(64, 54, 46)
	ramp.Material = Enum.Material.WoodPlanks
	ramp.Anchored = true
	ramp.Parent = battery

	-- 5. Wooden Pilings / Stilts driven down into the lakebed ("tiang kayu pelabuhan")
	local pilings = {
		-- Along shore walkway
		Vector3.new(deckCenterX - 3.4, BATTERY_Y - 3.5, -684),
		Vector3.new(deckCenterX + 3.4, BATTERY_Y - 3.5, -684),
		Vector3.new(deckCenterX - 3.4, BATTERY_Y - 3.5, -689),
		Vector3.new(deckCenterX + 3.4, BATTERY_Y - 3.5, -689),
		Vector3.new(deckCenterX - 3.4, BATTERY_Y - 3.5, -694),
		Vector3.new(deckCenterX + 3.4, BATTERY_Y - 3.5, -694),
		-- Under firing pier along Z = -695
		Vector3.new(-64, BATTERY_Y - 3.5, -695),
		Vector3.new(-44, BATTERY_Y - 3.5, -695),
		Vector3.new(-24, BATTERY_Y - 3.5, -695),
		Vector3.new(32, BATTERY_Y - 3.5, -695),
		Vector3.new(52, BATTERY_Y - 3.5, -695),
		Vector3.new(72, BATTERY_Y - 3.5, -695),
		-- Under main barge
		Vector3.new(-70, BATTERY_Y - 3.5, -707),
		Vector3.new(-35, BATTERY_Y - 3.5, -707),
		Vector3.new(43, BATTERY_Y - 3.5, -707),
		Vector3.new(78, BATTERY_Y - 3.5, -707),
	}
	for pIdx, pPos in ipairs(pilings) do
		local post = Instance.new('Part')
		post.Name = `PierPiling_{pIdx}`
		post.Shape = Enum.PartType.Cylinder
		post.Size = Vector3.new(9, 1.2, 1.2)
		post.CFrame = CFrame.new(pPos) * CFrame.Angles(0, 0, math.rad(90))
		post.Color = Color3.fromRGB(36, 28, 22)
		post.Material = Enum.Material.Wood
		post.Anchored = true
		post.Parent = battery
	end

	-- 6. Mooring bollards on the wooden dock corners for harbor detail
	local bollards = {
		Vector3.new(deckCenterX - 3.6, BATTERY_Y + 1.1, -683),
		Vector3.new(deckCenterX + 3.6, BATTERY_Y + 1.1, -683),
		Vector3.new(-70, BATTERY_Y + 1.1, -695),
		Vector3.new(74, BATTERY_Y + 1.1, -695),
		Vector3.new(-76, BATTERY_Y + 1.1, -714),
		Vector3.new(80, BATTERY_Y + 1.1, -714),
	}
	for bIdx, bPos in ipairs(bollards) do
		local bollard = Instance.new('Part')
		bollard.Name = `MooringBollard_{bIdx}`
		bollard.Shape = Enum.PartType.Cylinder
		bollard.Size = Vector3.new(1.0, 0.7, 0.7)
		bollard.CFrame = CFrame.new(bPos) * CFrame.Angles(0, 0, math.rad(90))
		bollard.Color = Color3.fromRGB(32, 34, 38)
		bollard.Material = Enum.Material.Metal
		bollard.Anchored = true
		bollard.Parent = battery
	end

	-- 7. Low protective timber curbs along the walkway edges
	local leftCurb = Instance.new('Part')
	leftCurb.Name = 'WalkwayLeftCurb'
	leftCurb.Size = Vector3.new(0.4, 0.4, walkwayLength)
	leftCurb.CFrame = CFrame.new(deckCenterX - 3.8, deckTopY + 0.2, walkwayCenterZ)
	leftCurb.Color = Color3.fromRGB(44, 36, 30)
	leftCurb.Material = Enum.Material.Wood
	leftCurb.Anchored = true
	leftCurb.Parent = battery

	local rightCurb = Instance.new('Part')
	rightCurb.Name = 'WalkwayRightCurb'
	rightCurb.Size = Vector3.new(0.4, 0.4, walkwayLength)
	rightCurb.CFrame = CFrame.new(deckCenterX + 3.8, deckTopY + 0.2, walkwayCenterZ)
	rightCurb.Color = Color3.fromRGB(44, 36, 30)
	rightCurb.Material = Enum.Material.Wood
	rightCurb.Anchored = true
	rightCurb.Parent = battery
...
local function collectLaunchOrigins(): { Vector3 }
	local batteryOrigins = buildLaunchBattery()
...
local function attachConsole(base: BasePart)
	...
	local TITLE_PREFIX = utf8.char(0x82B1, 0x706B, 0x5927, 0x4F1A, 0x20, 0x2022, 0x20)
	...
	label.Text = `{TITLE_PREFIX}Ready ({formatRemaining(showDuration())})`
	label.Parent = gui
	...
	local function triggerShow(startOffsetSecs: any)
		...
		task.spawn(function()
			while running and os.clock() < finishAt do
				...
				label.Text = `{TITLE_PREFIX}{formatRemaining(remaining)}`
